...
# Function to create ResNet50 model for 5-class classification
...
for fold, (train_index, test_index) in enumerate(kf.split(np.arange(5400))):
    ...
    layer_outputs = [layer.output for layer in model.layers[0].layers[:10]]  # Choose the first 10 layers for visualization
    activation_model = tf.keras.models.Model(inputs=model.layers[0].inputs, outputs=layer_outputs)
